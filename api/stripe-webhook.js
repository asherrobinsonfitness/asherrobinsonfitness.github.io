...
export default async function handler(req, res) {
    ...
    if (event.type === 'payment_intent.succeeded') {
        ...
        if (meta.ph_distinct_id) {
            try {
                await getPostHogClient().captureImmediate({
                    distinctId: meta.ph_distinct_id,
                ...
                console.error('[stripe-webhook] PostHog capture failed', err.message);
                // Same reasoning as the CAPI catch above — don't fail the webhook
                // over a delivery hiccup on an already-succeeded payment.
            }
        }
    }
